...
/**
 * Real-time meeting hook using Supabase Realtime Presence
 * No fake participants - only real users who join via invite link
 */
export function useRealtimeMeeting({
      ...
      const checkAudioLevel = () => {
        ...
        if (isSpeaking) {
          ...
          channelRef.current?.track({
            ...
            isVideoOn: localParticipant?.isVideoOn ?? true,
            isSpeaking: true,
            isHandRaised: localParticipant?.isHandRaised ?? false,
            joinedAt: localParticipant?.joinedAt?.toISOString() ?? new Date().toISOString(),
          });
        }
  ...
  // Track presence state to Supabase
  ...
  // Join meeting - gets local media and joins realtime channel
    ...
    try {
      ...
      await channel.subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.log('Joined meeting channel');
        }
      });
  ...
  }, [meetingId, userId, userName, isHost, isJoining, isJoined, setupAudioAnalysis, onMeetingJoined, onParticipantJoined, onParticipantLeft, onError]);
  ...
  const leaveMeeting = useCallback(async () => {
    ...
    if (waitingChannelRef.current) {
      await supabase.removeChannel(waitingChannelRef.current);
  ...
  const toggleMicrophone = useCallback(async () => {
    ...
    if (audioTrack) {
      ...
      setLocalParticipant((prev) => {
        if (!prev) return null;
        const updated = { ...prev, isMuted: newMuted };
  ...
  const toggleCamera = useCallback(async () => {
    ...
    if (videoTrack) {
      ...
      setLocalParticipant((prev) => {
        if (!prev) return null;
        const updated = { ...prev, isVideoOn: newVideoOn };
  ...
  // Lower all hands (host only) - broadcast via channel
  const lowerAllHands = useCallback(() => {
    if (!isHost) return;
    
    // Broadcast lower all hands event
  ...
  // Mute participant (host only) - broadcast via channel
  ...
  const admitParticipant = useCallback((participantId: string) => {
    ...
    if (waiting) {
      ...
      toast.success(`${waiting.userName} has been admitted`);
    }
  ...
  // Listen for broadcast events (mute, remove, etc.)
  useEffect(() => {
